...
extension ECPieChartConfig {
    ...
    fileprivate func computerArcCenterPoint() {
        let pointX : CGFloat = self.arcCenter.x + 0 * cos(self.endAngle - (CGFloat.pi * self.arcPercent))
        let pointY : CGFloat = self.arcCenter.y + 0 * sin(self.endAngle - (CGFloat.pi * self.arcPercent))
